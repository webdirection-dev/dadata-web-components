window.addEventListener('DOMContentLoaded', () => {
    ...
    function renderVariantList(valueInput) {
        ...
        arrForRender.forEach((item, index) => {
            ...
            if (index < 5) {
                ...
                elem.innerHTML = `
                        <div class=${classTitle}>${markLetters(value, valueInput)}</div>
                        <div class="variant__description">
                            <p class="variant__inn">${markLetters(data.inn, valueInput)}</p>
                            <p class="variant__own">${data.address === null ? '' : markLetters(data.address.value, valueInput)}</p>
                        </div>
                    `
    ...
    async function getIP() {
        let urlIp = 'http://api.ipstack.com/check?access_key=93744030badf23ec17f6c5613a7da447'
        try {
    ...
    async function getData(query) {
        ...
        try {
            ...
            const withCity = data.suggestions.filter(item => item.data.address.data.city === 'Москва')
            const withoutCity = data.suggestions.filter(item => item.data.address.data.city !== 'Москва')
